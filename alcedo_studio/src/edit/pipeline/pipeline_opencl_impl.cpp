...
class OpenCLGPUPipeline final : public GPUPipelineImpl {
 private:
  ...
  void EnsureOpenClInput() {
    ...
    if (!has_valid_gpu && !has_valid_cpu) {
      throw std::runtime_error(
          "OpenCL fused pipeline: input image has no valid CPU or GPU data.");
    }

    const bool needs_sync =
        !has_valid_gpu || input_img_->GetGPUBackend() != GpuBackendKind::OpenCL;
  ...
  void EnsureKernels() {
    ...
    if (program == nullptr) {
      throw std::runtime_error(
          "OpenCL fused pipeline: failed to get program from library.");
    }

    if (fused_kernel_ == nullptr) {
      cl_int err = CL_SUCCESS;
      fused_kernel_ =
          clCreateKernel(program, OpenCL::Pipeline::kFusedKernelName, &err);
  ...
  void ValidateParamsABI() {
    ...
    cl_int  err = CL_SUCCESS;
    cl_mem output_buffer = clCreateBuffer(context.Context(), CL_MEM_READ_WRITE,
                                          8 * sizeof(float), nullptr, &err);
    if (err != CL_SUCCESS || output_buffer == nullptr) {
      throw std::runtime_error(
          "OpenCL fused pipeline: failed to create validation output buffer.");
    }

    err             = CL_SUCCESS;
    cl_uint arg_index = 0;
    cl_mem params_buf = resources_.params_buffer_.Get();
    err |= clSetKernelArg(kernel, arg_index++, sizeof(cl_mem),
                          &params_buf);
    err |= clSetKernelArg(kernel, arg_index++, sizeof(cl_mem), &output_buffer);
    if (err != CL_SUCCESS) {
      clReleaseMemObject(output_buffer);
      throw std::runtime_error(
          "OpenCL fused pipeline: failed to set validation kernel arguments.");
    }

    size_t global_size = 1;
    err = clEnqueueNDRangeKernel(context.Queue(), kernel, 1, nullptr, &global_size,
                                  nullptr, 0, nullptr, nullptr);
    if (err != CL_SUCCESS) {
      clReleaseMemObject(output_buffer);
      throw std::runtime_error(
          "OpenCL fused pipeline: failed to enqueue validation kernel.");
    }
    ...
    if (err != CL_SUCCESS) {
      throw std::runtime_error(
          "OpenCL fused pipeline: failed to read validation output.");
    }

    clFinish(context.Queue());
  }

  void EnqueueFusedKernel(const opencl::OpenClImage& src, opencl::OpenClImage& dst) {
    auto& context = OpenClContext::Instance();
    if (!context.IsInitialized()) {
 ...
 public:
  ...
  void SetParams(OperatorParams& params) override {
    ...
    resources_ = OpenCL::Pipeline::OpenClFusedParamUploader::Upload(
        fused_params_, params, resources_);
  }
  ...
  void Execute(std::shared_ptr<ImageBuffer> output_img) override {
    ...
    EnqueueFusedKernel(input, working_);
    ...
    if (frame_sink_) {
      ...
      cv::Mat contiguous_host(host_image.rows, host_image.cols, CV_32FC4,
                              host_pixels->data(), row_bytes);
