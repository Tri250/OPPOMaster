...
auto IsZeroHash(const version_id_t& id) -> bool {
...
auto NowTime() -> std::time_t {
  return std::chrono::system_clock::to_time_t(TimeProvider::Now());
}

auto EmptyTransactions() -> const std::vector<EditTransaction>& {
  static const std::vector<EditTransaction> txs;
  return txs;
}
}  // namespace
...
auto Version::Checkpoint(sl_element_id_t bound_image, version_id_t parent_version_id,
                         ...
                         std::optional<EditTransaction> last_transaction) -> Version {
...
void Version::SetBoundImage(sl_element_id_t image_id) {
...
void Version::AppendEditTransaction(EditTransaction&& /*edit_transaction*/) {
  throw std::runtime_error("Version is a committed checkpoint; append to WorkingVersion instead");
}

auto Version::RemoveLastEditTransaction() -> EditTransaction {
  throw std::runtime_error("Version is a committed checkpoint; remove from WorkingVersion instead");
}

auto Version::GetTransactionByID(tx_id_t /*transaction_id*/) -> EditTransaction& {
  throw std::runtime_error("Version checkpoints do not store transaction lists");
}
