...
static inline float LoadPlane(global const float* plane, constant SinglePlaneParams* params, int y,
                              int x) {
  return plane[(uint)y * params->stride + (uint)x];
}

static inline float LoadDir(global const float* dir, constant SinglePlaneParams* params, int y,
                            int x) {
  return dir[(uint)y * params->stride + (uint)x];
}
...
static inline float LowPassAt(global const float* raw, constant SinglePlaneParams* params, int y,
                              int x) {
...
static inline float ReconstructRbAtGreen(global const float* channel,
...
__kernel void rcd_init_and_vh(global const float* raw,
                              ...
                              global float*       b,
                              global float*       vh_dir,
                              constant SinglePlaneParams* params) {
  ...
  if (x >= 4u && y >= 4u && x + 4u < params->width && y + 4u < params->height) {
    ...
    float vm1 = LoadPlane(raw, params, iy - 1, ix);
    float vp1 = LoadPlane(raw, params, iy + 1, ix);
    float vm2 = LoadPlane(raw, params, iy - 2, ix);
    float vp2 = LoadPlane(raw, params, iy + 2, ix);
    float vm3 = LoadPlane(raw, params, iy - 3, ix);
    float vp3 = LoadPlane(raw, params, iy + 3, ix);
    float vm4 = LoadPlane(raw, params, iy - 4, ix);
    float vp4 = LoadPlane(raw, params, iy + 4, ix);

    float hm1 = LoadPlane(raw, params, iy, ix - 1);
    float hp1 = LoadPlane(raw, params, iy, ix + 1);
    float hm2 = LoadPlane(raw, params, iy, ix - 2);
    float hp2 = LoadPlane(raw, params, iy, ix + 2);
    float hm3 = LoadPlane(raw, params, iy, ix - 3);
    float hp3 = LoadPlane(raw, params, iy, ix + 3);
    float hm4 = LoadPlane(raw, params, iy, ix - 4);
    float hp4 = LoadPlane(raw, params, iy, ix + 4);
...
__kernel void rcd_green_at_rb(global const float* raw,
  ...
  float VH_central = LoadDir(vh_dir, params, iy, ix);
  float VH_neigh   = 0.25f * (LoadDir(vh_dir, params, iy - 1, ix - 1) +
                              LoadDir(vh_dir, params, iy - 1, ix + 1) +
                              LoadDir(vh_dir, params, iy + 1, ix - 1) +
                              LoadDir(vh_dir, params, iy + 1, ix + 1));
  float VH_disc =
      (fabs(0.5f - VH_central) < fabs(0.5f - VH_neigh)) ? VH_neigh : VH_central;

  float c   = LoadPlane(raw, params, iy, ix);
  float vm1 = LoadPlane(raw, params, iy - 1, ix);
  float vp1 = LoadPlane(raw, params, iy + 1, ix);
  float vm2 = LoadPlane(raw, params, iy - 2, ix);
  float vp2 = LoadPlane(raw, params, iy + 2, ix);
  ...
  float lpf_c  = LowPassAt(raw, params, iy, ix);
  float lpf_n2 = LowPassAt(raw, params, iy - 2, ix);
  float lpf_s2 = LowPassAt(raw, params, iy + 2, ix);
  float lpf_w2 = LowPassAt(raw, params, iy, ix - 2);
  float lpf_e2 = LowPassAt(raw, params, iy, ix + 2);
  ...
  g[y * params->stride + x] = ClampRcdOutput(VH_disc * H_est + (1.f - VH_disc) * V_est);
}
...
__kernel void rcd_pq_dir(global const float* raw,
  ...
  if (x >= 4u && y >= 4u && x + 4u < params->width && y + 4u < params->height &&
    ...
    float nw1 = LoadPlane(raw, params, iy - 1, ix - 1);
    float se1 = LoadPlane(raw, params, iy + 1, ix + 1);
    float nw2 = LoadPlane(raw, params, iy - 2, ix - 2);
    float se2 = LoadPlane(raw, params, iy + 2, ix + 2);
    float nw3 = LoadPlane(raw, params, iy - 3, ix - 3);
    float se3 = LoadPlane(raw, params, iy + 3, ix + 3);
    float nw4 = LoadPlane(raw, params, iy - 4, ix - 4);
    float se4 = LoadPlane(raw, params, iy + 4, ix + 4);

    float sw1 = LoadPlane(raw, params, iy + 1, ix - 1);
    float ne1 = LoadPlane(raw, params, iy - 1, ix + 1);
    float sw2 = LoadPlane(raw, params, iy + 2, ix - 2);
    float ne2 = LoadPlane(raw, params, iy - 2, ix + 2);
    float sw3 = LoadPlane(raw, params, iy + 3, ix - 3);
    float ne3 = LoadPlane(raw, params, iy - 3, ix + 3);
    float sw4 = LoadPlane(raw, params, iy + 4, ix - 4);
    float ne4 = LoadPlane(raw, params, iy - 4, ix + 4);
...
__kernel void rcd_rb_at_rb(global const float* pq_dir,
  ...
  float PQ_c    = LoadDir(pq_dir, params, iy, ix);
  float PQ_n    = 0.25f * (LoadDir(pq_dir, params, iy - 1, ix - 1) +
                           LoadDir(pq_dir, params, iy - 1, ix + 1) +
                           LoadDir(pq_dir, params, iy + 1, ix - 1) +
                           LoadDir(pq_dir, params, iy + 1, ix + 1));
  float PQ_disc = (fabs(0.5f - PQ_c) < fabs(0.5f - PQ_n)) ? PQ_n : PQ_c;

  float g_c     = LoadPlane(g, params, iy, ix);

  global const float* channel = (c == 0u) ? r : b;

  float ch_nw1 = LoadPlane(channel, params, iy - 1, ix - 1);
  float ch_ne1 = LoadPlane(channel, params, iy - 1, ix + 1);
  float ch_sw1 = LoadPlane(channel, params, iy + 1, ix - 1);
  float ch_se1 = LoadPlane(channel, params, iy + 1, ix + 1);

  float ch_nw3 = LoadPlane(channel, params, iy - 3, ix - 3);
  float ch_ne3 = LoadPlane(channel, params, iy - 3, ix + 3);
  float ch_sw3 = LoadPlane(channel, params, iy + 3, ix - 3);
  float ch_se3 = LoadPlane(channel, params, iy + 3, ix + 3);

  float g_nw2  = LoadPlane(g, params, iy - 2, ix - 2);
  float g_ne2  = LoadPlane(g, params, iy - 2, ix + 2);
  float g_sw2  = LoadPlane(g, params, iy + 2, ix - 2);
  float g_se2  = LoadPlane(g, params, iy + 2, ix + 2);
  ...
  float g_nw1 = LoadPlane(g, params, iy - 1, ix - 1);
  float g_ne1 = LoadPlane(g, params, iy - 1, ix + 1);
  float g_sw1 = LoadPlane(g, params, iy + 1, ix - 1);
  float g_se1 = LoadPlane(g, params, iy + 1, ix + 1);
  ...
  if (c == 0u) {
    r[y * params->stride + x] = out_val;
  } else {
    b[y * params->stride + x] = out_val;
  }
}
...
__kernel void rcd_rb_at_g(global const float* vh_dir,
  ...
  float g_c  = LoadPlane(g, params, iy, ix);
  float g_m2 = LoadPlane(g, params, iy - 2, ix);
  float g_p2 = LoadPlane(g, params, iy + 2, ix);
  float g_l2 = LoadPlane(g, params, iy, ix - 2);
  float g_r2 = LoadPlane(g, params, iy, ix + 2);
  float g_m1 = LoadPlane(g, params, iy - 1, ix);
  float g_p1 = LoadPlane(g, params, iy + 1, ix);
  float g_l1 = LoadPlane(g, params, iy, ix - 1);
  float g_r1 = LoadPlane(g, params, iy, ix + 1);

  uint index = y * params->stride + x;

  r[index] = ReconstructRbAtGreen(r, params, iy, ix, g_c, g_m2, g_p2, g_l2, g_r2, g_m1, g_p1,
                                  g_l1, g_r1, VH_disc);
  b[index] = ReconstructRbAtGreen(b, params, iy, ix, g_c, g_m2, g_p2, g_l2, g_r2, g_m1, g_p1,
                                  g_l1, g_r1, VH_disc);
}
...
__kernel void rcd_merge_rgba(global const float* r,
                             ...
                             constant SinglePlaneParams* in_params,
                             constant MergeParams*       out_params) {
  uint x = get_global_id(0);
  uint y = get_global_id(1);
  ...
  out_rgba[out_idx] =
      (float4)(r[in_idx], g[in_idx], b[in_idx], 1.0f);
}
