...
void EditorDialog::ApplyStateToPipeline(const AdjustmentState& render_state) {
  ...
  auto params_for_field = [this](AdjustmentField        field,
                                 const AdjustmentState& state) -> nlohmann::json {
    return adjustment_session_
               ? adjustment_session_->ParamsForField(field, state)
               : pipeline_io::ParamsForField(field, state,
  ...
  loading.SetOperator(OperatorType::RAW_DECODE,
                      params_for_field(AdjustmentField::RawDecode, render_state_sanitized));
  loading.SetOperator(OperatorType::LENS_CALIBRATION,
                      params_for_field(AdjustmentField::LensCalib, render_state_sanitized),
                      global_params);
  ...
  output.SetOperator(OperatorType::ODT,
                     params_for_field(AdjustmentField::Odt, render_state_sanitized), global_params);
  ...
  if (active_panel_ == ControlPanelKind::Geometry) {
    ...
    crop_rotate_params = params_for_field(AdjustmentField::CropRotate, committed_state_);
  }
  ...
  basic.SetOperator(OperatorType::SHADOWS, {{"shadows", render_state_sanitized.shadows_}},
                    global_params);
  basic.SetOperator(OperatorType::HIGHLIGHTS, {{"highlights", render_state_sanitized.highlights_}},
                    global_params);
  ...
  color.SetOperator(OperatorType::SATURATION, {{"saturation", render_state_sanitized.saturation_}},
                    global_params);
  color.EnableOperator(OperatorType::TINT, false, global_params);
  color.SetOperator(OperatorType::COLOR_WHEEL,
                    params_for_field(AdjustmentField::ColorWheel, render_state_sanitized),
                    global_params);
  color.EnableOperator(OperatorType::COLOR_WHEEL, true, global_params);
  color.SetOperator(OperatorType::HLS,
                    params_for_field(AdjustmentField::Hls, render_state_sanitized), global_params);
  ...
  detail.SetOperator(OperatorType::SHARPEN,
                     {{"sharpen", {{"offset", render_state_sanitized.sharpen_}}}}, global_params);
