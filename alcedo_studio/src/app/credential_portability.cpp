...
// AES-256-GCM key derivation using PBKDF2 with SHA-256
constexpr int    kKeyLength     = 32;   // 256 bits
constexpr int    kIVLength      = 12;   // 96 bits for GCM
...
auto DeriveKeyFromPassword(const std::string& password, const std::string& salt)
    -> std::vector<uint8_t> {
  // PBKDF2-HMAC-SHA256 implementation using QCryptographicHash
  ...
  // PRF = HMAC-SHA256, dkLen = kKeyLength, c = kPBKDF2Iter
  ...
  for (int block = 1; block <= blocks && result.size() < dkLen; ++block) {
    ...
    // HMAC-SHA256: H(key ^ opad || H(key ^ ipad || data))
  ...
  result.resize(dkLen);
  return std::vector<uint8_t>(result.constBegin(), result.constEnd());
}

// Simple AES-256-GCM-like encryption using XOR stream with HMAC for authentication.
// For production, replace with a proper crypto library (OpenSSL, libsodium).
auto XorStreamCipher(const std::string& plaintext, const std::vector<uint8_t>& key,
                     const std::string& iv) -> std::string {
  std::string result(plaintext.size(), '\0');
  // Generate a pseudo-random stream from key + iv using SHA-256 rounds
  QByteArray seed;
  seed.append(reinterpret_cast<const char*>(key.data()), static_cast<qsizetype>(key.size()));
  seed.append(iv.data(), static_cast<qsizetype>(iv.size()));

  size_t offset = 0;
  while (offset < plaintext.size()) {
    QCryptographicHash hash(QCryptographicHash::Sha256);
    hash.addData(seed);
    // Add counter to produce different blocks
    hash.addData(QByteArray(1, static_cast<char>(offset / 32)));
    QByteArray block = hash.result();

    for (int i = 0; i < block.size() && offset < plaintext.size(); ++i, ++offset) {
      result[offset] = plaintext[offset] ^ block[i];
    }
  }
  return result;
}

auto ComputeHMAC(const std::string& data, const std::vector<uint8_t>& key) -> std::string {
  // Simple HMAC-SHA256 for authentication
  QCryptographicHash hash(QCryptographicHash::Sha256);
  QByteArray key_block(reinterpret_cast<const char*>(key.data()),
                       static_cast<qsizetype>(key.size()));

  // HMAC: H(key ^ opad || H(key ^ ipad || data))
  QByteArray ipad(64, static_cast<char>(0x36));
  QByteArray opad(64, static_cast<char>(0x5c));
  for (int i = 0; i < key_block.size() && i < 64; ++i) {
    ipad[i] = ipad[i] ^ key_block[i];
    opad[i] = opad[i] ^ key_block[i];
  }
  ...
  QCryptographicHash inner(QCryptographicHash::Sha256);
  inner.addData(ipad);
  inner.addData(data.data(), static_cast<qsizetype>(data.size()));

  QCryptographicHash outer(QCryptographicHash::Sha256);
  outer.addData(opad);
  outer.addData(inner.result());

  return outer.result().toStdString();
}
...
auto CredentialPortability::EncryptSecret(const std::string& secret,
  ...
  // Derive key from password (or use a fixed key for no-password mode)
  ...
  // Encrypt
  std::string encrypted = XorStreamCipher(secret, key, iv_out);

  // Compute authentication tag
  tag_out = ComputeHMAC(encrypted, key);
...
auto CredentialPortability::DecryptSecret(const std::string& encrypted,
  ...
  std::string computed_tag = ComputeHMAC(ciphertext, key);
  if (computed_tag != tag) {
    return std::nullopt;  // Authentication failed
  }

  return XorStreamCipher(ciphertext, key, iv);
}
