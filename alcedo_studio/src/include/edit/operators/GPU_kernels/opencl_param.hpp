...
struct OpenClFusedResources {
  ...
  void Reset() { params_buffer_.Reset(); }
};

class OpenClFusedParamUploader {
 public:
  static auto Upload(const FusedOperatorParams& fused_params, const OperatorParams& cpu_params,
    ...
    resources.opencl_params_       = BuildOpenClParams(fused_params, cpu_params);
    resources.params_buffer_       = OpenClBuffer::CreateReadOnlyCopy(&resources.opencl_params_,
                                                                      sizeof(OpenClFusedParams));
    return resources;
  }

 private:
  static auto BuildOpenClParams(const FusedOperatorParams& fused_params,
                                const OperatorParams& cpu_params) -> OpenClFusedParams {
