...
GPU_FUNC float hs_shadow_llf_detail_gain(float detail, float shadow_amount, float shadow_zone) {
  ...
  const float     limited_gain = fminf(fmaxf(remap_gain - 1.0f, 0.0f), 0.38f);
...
GPU_FUNC float4 hs_apply_local_tone_pixel(float4 px, float base, GPUOperatorParams params) {
  ...
  const float llf_detail_gain =
      hs_shadow_llf_detail_gain(
          ...
          fmaxf(shadow_texture_zone, 0.86f * shadow_fill_light_zone * fill_highlight_guard));
  ...
  const float fill_light_recovery =
      (0.075f + 0.085f * fmaxf(base_contrast_loss, shadow_contrast_loss));
  ...
  const float raw_detail_scale = fminf(
      1.30f, fmaxf(0.97f, (1.0f + shadow_detail_scale + highlight_detail_scale) * llf_detail_gain));
  ...
  const float local_mix =
      ...
      hs_local_detail_reference_guard(detail, tonal_reference_mix);
