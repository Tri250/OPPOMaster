...
TEST(HalationOpTest, GlobalParamsWritesHiddenDefaultsAndNormalizedStrength) {
  ...
  EXPECT_FLOAT_EQ(params.halation_.low_threshold_, 0.6f);
  EXPECT_FLOAT_EQ(params.halation_.high_threshold_, 0.7f);
  EXPECT_FLOAT_EQ(params.halation_.sigma_, 20.0f);
  ...
  EXPECT_FLOAT_EQ(params.halation_.additive_scale_, 1.0f);
}
...
TEST(HalationOpTest, EnableGlobalParamsTogglesHalationPayload) {
  ...
  EXPECT_FLOAT_EQ(params.halation_.strength_, 0.25f);
...
TEST(HalationOpTest, FusedParamsCarryHalationPayload) {
  ...
  EXPECT_FLOAT_EQ(fused.halation_.strength_, 0.75f);
  EXPECT_FLOAT_EQ(fused.halation_.low_threshold_, 0.6f);
  EXPECT_FLOAT_EQ(fused.halation_.high_threshold_, 0.7f);
  EXPECT_FLOAT_EQ(fused.halation_.sigma_, 20.0f);
  ...
  EXPECT_FLOAT_EQ(fused.halation_.additive_scale_, 1.0f);
}
