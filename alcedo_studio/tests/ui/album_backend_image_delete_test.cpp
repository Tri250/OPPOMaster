...
TEST_F(DeleteTests, AddToAlbumThenDeleteFromAlbum_KeepsRootFile) {
  ...
  backend.CreateFolder("AlbumA");
  ProcessEvents(500);
  const uint album_ui_id = FindFolderId(backend.Folders(), "AlbumA");
  ...
  const QVariantMap add_result = backend.AddImagesToFolder(targets, album_ui_id);
  ...
  ProcessEvents(500);
  ASSERT_EQ(backend.ShownCount(), 1);
  const QVariantMap root_after_unlink = backend.Thumbnails().front().toMap();
