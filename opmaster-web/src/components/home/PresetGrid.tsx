...
export default function PresetGrid() {
  ...
  return (
    <section className="py-16 px-page sm:px-page lg:px-page max-w-7xl mx-auto">
      <motion.div
      ...
      >
        {/* 一级标题使用哈苏橙 */}
        <h2 className="text-3xl md:text-4xl font-bold mb-3 text-hasselblad">
          精选影像推荐
        </h2>
        <p className="text-base text-text-tertiary max-w-xl mx-auto">
            ...
            <motion.button
              ...
              className={`px-5 py-2 rounded-small text-sm font-medium transition-all ${
                filterType === option.type
                  ? 'bg-oppo-primary text-text-primary'
      ...
      {isLoading ? (
        ...
        >
          <div className="w-12 h-12 border-4 border-oppo-primary/30 border-t-oppo-primary rounded-full animate-spin mx-auto mb-4" />
          <p className="text-text-tertiary">加载影像数据中...</p>
        </motion.div>
      ) : filteredPresets.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-feature-card">
        ...
        <motion.div
          ...
          className="text-center py-20"
        >
          <Filter className="w-16 h-16 text-text-tertiary/30 mx-auto mb-4" />
          <p className="text-xl text-text-tertiary">没有找到匹配的影像</p>
